...
def _guide_nav_keyboard(topic: str, page: int, total: int) -> InlineKeyboardMarkup:
...
def _topic_keyboard(guide_topic: str) -> InlineKeyboardMarkup:
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not help_text and not guide_topic:
        await update.effective_message.reply_text(
            f"No help for '{topic}'. Try /help to see all commands."
        )
        return
    ...
    if guide_topic:
        await update.effective_message.reply_text(
            text,
            reply_markup=_topic_keyboard(guide_topic),
        )
    else:
...
async def handle_guide_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await query.edit_message_text(
            header + text,
            reply_markup=_guide_nav_keyboard(topic, page, total),
        )
    except BadRequest as exc:
        if "Message is not modified" not in str(exc):
            raise
